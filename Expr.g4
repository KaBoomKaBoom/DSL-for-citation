...
type : 'int' | 'string';
statement :  (var_assign NEWLINE)+ | (expr NEWLINE)+ | 'for' SPACE id SPACE 'in' SPACE id ':' NEWLINE expr+| 'break' ;
method_name : 'CiteAPA' | 'CiteMLA' | 'CiteCMS' | 'CiteCSE' | 'CiteISO' | 'CiteIEEE' | 'GenImage';
expr : method_call | literal |'print(' id ')' | id | (literal ',' literal)  ;
method_call : method_name '(' (literal | id) ',' SPACE ((literal | id) (',' SPACE)?)* ')';
var_assign: id SPACE assign_op SPACE literal |
            id SPACE assign_op SPACE '[' string_literal','SPACE? (string_literal ','? SPACE?)*  ']' | 
